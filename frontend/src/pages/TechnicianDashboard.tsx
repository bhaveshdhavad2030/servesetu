    ...
    <div className="min-h-screen bg-slate-50">
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between gap-4">
          ...
          <div className="text-center">
            <p className="text-xs uppercase tracking-[0.3em] text-[#2563EB]">Technician Dashboard</p>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="grid gap-6 lg:grid-cols-4 mb-10">
          {[
            { label: "Jobs Completed", value: "42", icon: <Briefcase className="w-6 h-6 text-white" />, color: "bg-[#0B3D91]" },
            { label: "Rating", value: "4.9★", icon: <Star className="w-6 h-6 text-white" />, color: "bg-[#2563EB]" },
        ...
        <div className="grid gap-8 xl:grid-cols-[1.4fr_0.85fr]">
          <section className="rounded-[2rem] bg-white border border-slate-200 p-8 shadow-sm">
            ...
            <div className="space-y-4">
              {jobs.map((job) => (
                <div key={job.id} className="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-5 hover:border-[#2563EB] transition">
